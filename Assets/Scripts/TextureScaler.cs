...
public class TextureScaler
{
    ...
    public static Texture2D scaled(WebCamTexture src, int width, int height, FilterMode mode = FilterMode.Trilinear)
    {
        Rect texR = new Rect(0,0,width,height);
        _gpu_scale(src,width,height,mode);
    ...
    static void _gpu_scale(WebCamTexture src, int width, int height, FilterMode fmode)
    {
        //We need the source texture in VRAM because we render with it
        src.filterMode = fmode;
        // src.Apply(true);    
